...
export default function Blog() {
  ...
  // Função para buscar os artigos do banco
  async function fetchArtigos() {
    const response = await fetch("/api/verArtigos");
    const data = await response.json();
    
    // Ordena os artigos por data de criação, do mais recente para o mais antigo
          ...
          {paginatedArtigos.map((artigo) => (
            ...
            >
              ...
              <p className="text-gray-600">
                {artigo.texto.slice(0, 100)}... {/* Exibe uma prévia do texto */}
              </p>
